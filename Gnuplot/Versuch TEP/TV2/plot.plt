...
set title 'Isochore Zustandsänderung bei $h = 90 \ \mathrm{mm}$'
set ylabel '$p/p_0$'
...
set fit quiet
fit f(x) 'data.csv' using 1:($3/p0) via a,b 
...
stats 'data.csv' using 1:($3/p0) name 'T' nooutput   # gives T_correlation
stats 'data.csv' using ($3/p0) name 'Yt' nooutput          # single-expression stats (Yt_stddev)
SST = (Yt_records > 1) ? (Yt_records - 1) * Yt_stddev**2 : 0
stats 'data.csv' using (f($1) - ($3/p0)) name 'RES' nooutput
SSE = RES_sumsq
...
plot \
	'data.csv' using 1:($3/p0) with points ls 2 title 'Messdaten', \
...
print sprintf('Analysis:      T0 = 1/a = %.6f +- %.6f (%.2f%%)', T0, T0_err, (T0!=0)?100.0*T0_err/abs(T0):0/0)
