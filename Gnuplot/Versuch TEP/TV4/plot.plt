...
set title ' Isobare Zustandsänderung bei $p = p_0 = 716 \ \mathrm{mmHg}$'
set ylabel '$V/V_0$'
set xlabel '$\Theta \ \mathrm{(^\circ C)}$'
# set grid
...
set fit quiet
fit f(x) 'data.csv' using 2:($1/xA) via a,b 
...
stats 'data.csv' using 1:($1/xA) name 'T' nooutput   # gives T_correlation
stats 'data.csv' using ($1/xA) name 'Yt' nooutput          # single-expression stats (Yt_stddev)
SST = (Yt_records > 1) ? (Yt_records - 1) * Yt_stddev**2 : 0
stats 'data.csv' using (f($2) - ($1/xA)) name 'RES' nooutput
SSE = RES_sumsq
...
# Derived parameter and its uncertainty
T0 = 1.0/a
T0_err = a_err/(a*a)
...
plot \
	'data.csv' using 2:($1/xA) with points ls 2 title 'Messdaten', \
...
print sprintf('Goodness:      R^2 = %.6f, r = %.6f,  N=%d', R2, r, Yt_records)
print sprintf('Analysis:      T0 = 1/a = %.6f +- %.6f (%.2f%%) K', T0, T0_err, (T0!=0)?100.0*T0_err/abs(T0):0/0)
