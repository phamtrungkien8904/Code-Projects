...
omega_d = 1.5    # driving angular frequency
...
def simulate():
	...
	rows = []
	for t in t_values:
		driving = F0 * np.cos(omega_d * t)
		# Mechanical energy (kinetic + potential); damping dissipates but we record instantaneous.
		energy = 0.5 * m * v * v + 0.5 * k * x * x
		rows.append((t, x, v, a, driving, energy))
		# Advance
		x, v, a = step(x, v, t)
	return rows
