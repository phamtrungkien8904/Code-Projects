...
m_err = 0.1e-3 # Fehler der Masse einer Murmel in kg
d = 31.4e-3 # Durchmesser des Gefäßes in m
d_err = 0.05e-3 # Fehler des Durchmessers in m

rho0 = 4*m/(a*pi*d**2) # Dichte der Flüssigkeit in kg/m^3
rho0_err = rho0 * sqrt((a_err/a)**2 + (2* d_err/d)**2 + (m_err/m)**2 ) # Fehler der Dichte in kg/m^3
